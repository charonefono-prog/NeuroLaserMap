...
export async function getAdminUsers(): Promise<{ users: any[] }> {
  return apiCall<{ users: any[] }>("/api/trpc/users.getAdminUsers");
}

export async function updateUserApproval(userId: number, isApproved: boolean): Promise<{ success: boolean }> {
  return apiCall<{ success: boolean }>("/api/trpc/users.updateUserApproval", {
    method: "POST",
    body: JSON.stringify({ userId, isApproved }),
  });
}

export async function updateUserRole(userId: number, role: "user" | "professional" | "admin"): Promise<{ success: boolean }> {
  return apiCall<{ success: boolean }>("/api/trpc/users.updateUserRole", {
    method: "POST",
