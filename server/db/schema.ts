...
export const users = mysqlTable("users", {
  ...
  role: varchar("role", { length: 50 }).notNull().default("user"), // user, professional, admin
});
...
export const accessControl = mysqlTable("access_control", {
  ...
  accessLevel: varchar("access_level", { length: 50 }).notNull().default("user"), // user, professional, admin
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
});
